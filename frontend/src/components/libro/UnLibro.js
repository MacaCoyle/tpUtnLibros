...
export default function UnLibro() {
  ...
  useEffect(() => {
    async function connect() {
      try {
        const response = await axios.get(
          "http://localhost:3001/categoria/" + libro.categoria_id
        );
  ...
  useEffect(() => {
    if (libro.persona_id) {
      async function connect() {
        try {
          const response = await axios.get(
            "http://localhost:3001/persona/" + libro.persona_id
          );
    ...
    async function connect() {
      try {
        const response = await axios.put(
          "http://localhost:3001/libro/devolver/" + libro.libro_id
        );
  ...
  return (
    <div>
      ...
      <p>Categoria: {categoria && categoria.nombre}</p>
      <p>
        ¿Esta prestado? {persona.persona_id ? "SI" : "NO"}
        <br />
        Persona: {persona.persona_id && persona.nombre}{" "}
        {persona.persona_id && persona.apellido} <br />
